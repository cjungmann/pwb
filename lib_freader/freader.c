 ...
 * itself.
 */
EXPORT void destroy_lindex(LINDEX *index)
{
   if (index)
   {
      if (index->larray)
 ...
 */
EXPORT LINDEX *index_lines(FILE *stream)
{
...
EXPORT int get_lindex_row_count(const LINDEX *lindex)
{
...
EXPORT const char *get_lindex_line(const LINDEX *lindex, int index)
{
